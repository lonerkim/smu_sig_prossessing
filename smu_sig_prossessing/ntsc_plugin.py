...
import os as _os
_RING_PATH = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), 'ringPattern.npy')
RingPattern = np.load(_RING_PATH)
...
def ringing(img2d, alpha=0.5, noiseSize=0, noiseValue=2, clip=True, seed=None):
    ...
    if noiseSize > 0:
        ...
        mask = mask.astype(np.float64) + rnd.rand(mask.shape[0], mask.shape[1], mask.shape[2]) * noise - noise / 2.
